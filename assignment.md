...
● Sidebar (Left): A slim, collapsible navigation menu.
● Main Content (Right): A dynamic grid containing the following "Bento" tiles:
○ Hero Tile: A large greeting tile ("Welcome back, [Name]") with a daily learning streak
indicator.
○ Course Tiles (Dynamic): Smaller tiles displaying active courses fetched from your
database.
○ Activity Tile: A medium tile showing a mock contribution graph or activity chart.
2. The Tech Stack & Constraints
You must use the following tools. Substitutions are not permitted for this assignment.
...
The "feel" of this app is just as important as the look. We expect the following interactions
handled via Framer Motion:
● Staggered Page Load: Once the data is fetched and rendered, the Bento tiles should not
appear all at once. They must stagger in sequentially (e.g., fading in while translating
...
1. Icon: Dynamically rendered based on the icon_name field from the database.
...
card content.
6. Responsive Design
● Desktop (> 1024px): Full Bento grid with the sidebar visible.
● Tablet (768px - 1024px): Sidebar collapses to icons only. Bento grid adjusts to a
2-column layout.
● Mobile (< 768px): Sidebar moves to a bottom navigation bar or hamburger menu. The
Bento grid stacks into a single, vertical scrolling column. 🏆 Evaluation Rubric
We will review your code based on the following criteria:
1. Data Architecture & Next.js (30%): Did you successfully implement Server
...
modern application? Does the layout degrade gracefully on mobile?
How to Submit
1. Host your code on a public GitHub repository.
2. Deploy the application using Vercel.
3. Include a .env.example file so we know which Supabase environment variables are
required. Do NOT commit your actual .env file.
4. Include a brief README.md explaining your architectural choices, how you handled the
server/client component split, and any challenges you faced.